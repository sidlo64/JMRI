...
```
  ./scripts/update-HOWTO.sh 5.15.5 5.15.6 5.15.7
```
...
- To check the script ran OK, the following should be the release you're doing now: 5.15.5
...
- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 5.15.5". (This [might already exist](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.15.5%22), if it was properly created at the end of the last build cycle)  Typical content:
```
...
```
        git commit -m"for 5.15.5" scripts/HOWTO-distribution.md
        git push github
```
...
```
        ...
        git checkout master
        git pull
        cp jmri5.15.5.shtml jmri5.15.6.shtml
        $EDITOR jmri5.15.6.shtml
```
...
```
        git add jmri5.15.6.shtml
        git commit -m"start new 5.15.6 next release note" jmri5.15.6.shtml
        git push github
        cd ../../JMRI
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri5.15.5.shtml
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri5.15.5.shtml
```

 - Add any new warnings to the old warnings section of the next (5.15.6) release note:
```
        ${EDITOR} ../website/releasenotes/jmri5.15.5.shtml ../website/releasenotes/jmri5.15.6.shtml
```

- Clean out the unneeded sections from the release note
```
        ${EDITOR} ../website/releasenotes/jmri5.15.5.shtml
```
...
```
        ...
        git commit -m"start for 5.15.6 release note" help/en/releasenotes/*.shtml
        git push github
        git pull
```
...
```
        ...
        git commit -m"updated 5.15.5 release notes" jmri5.*
        git push github
        git pull
        cd ../../JMRI
```
...
```
        git commit -m"5.15.5 until next release" release.properties pom.xml
```
...
```
        ...
        git commit -m"SBOM update for 5.15.5" lib/bom-Java-Maven.spdx
...
- Remove the 5.15.5 milestone on [unmerged PRs or [click here]](https://github.com/JMRI/JMRI/pulls?q=is%3Aopen+is%3Apr+milestone%3A5.15.5)

- Check on GitHub that [`release.properties`](https://github.com/JMRI/JMRI/blob/master/release.properties) on `master` is showing 5.15.5

- Create a `release-5.15.5` branch on GitHub
```
        git checkout -b release-5.15.5
        git push github
```

- Put the following comment in the [release GitHub item](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.15.5%22) saying the branch exists, and all future changes should be documented in the new release note: (NOT FOR THE LAST TEST RELEASE FROM MASTER BEFORE A PRODUCTION RELEASE, see just below)

```
The release-5.15.5 branch has been created.

Maintainers, please set the 5.15.6 milestone on pulls from now on, as that will be the next test release.

Jenkins will be creating files shortly at the [CI server](https://builds.jmri.org/jenkins/job/testreleases/job/5.15.5/)
```
...
Maintainers, please set the (next series, i.e. 5.15.6) milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.

Jenkins will be creating files shortly at the [CI server](https://builds.jmri.org/jenkins/job/TestReleases/job/5.15.5/)

If you're developing any additional (post-5.15.5) changes that you want in the JMRI 5.16 production release, please start from this branch, i.e. do `git fetch; git checkout release-5.15.5; git checkout -b myBranchName` to start your work.
```


- Fetch back to make sure your repository is fully up to date but stay on the release-5.15.5 branch
...
- Click "Copy Existing Item". Fill out the new 5.15.5 release name at the top. Enter the 5.15.5 most recent release at the bottom.  Click "OK"

- Update

        Description
        Source Code Management:
           Branch Specified:  5.15.5
...
- When that's done, put a screen-shot of the four monitor graphs into the "[Create Test Release 5.15.5](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.15.5%22)" Github issue so that historical resource usage info is available.

================================================================================
## Put Files Out For Checking

- Change the release note to point to the just-built files (in CI or where you put them), commit, wait (or force via ["Build Now"](https://builds.jmri.org/jenkins/job/website/job/website-repository/) button). Confirm [visible on web](https://www.jmri.org/releasenotes/jmri5.15.5.shtml).

- Announce the file set via [email to jmri@jmri-developers.groups.io](mailto:jmri@jmri-developers.groups.io?subject=First%205.15.5%20files%20available) with a subject line


```
"First 5.15.5 files available":

First JMRI 5.15.5 files are available in the usual way at:
...
These instructions result in the initial release-5.15.5 branch label being attached to the master branch sequence.  If somebody commits to master after that, master will move along with additional contents. Once that happens, if you want to include _all_ merges to master to date, you can just move the release-5.15.5 branch forward on master to the current head (and update the release note, decoderIndex and help files as needed)

If you want to add something to release-5.15.5 and _not_ include other additions to master, you have to branch the contents of release-5.15.5 away from master.  This section describes how to do that.

- Ideally, start the work on either the release-5.15.5 branch (if working after that was started) or on a branch-from-master that's _before_ the release-5.15.5 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:
   ...
   - One to the release branch e.g. "release-5.15.5".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- If somebody has merged their change into master (or it's branched from master later than the release tag), you have two choices:
   ...
   - Merge master into the release-5.15.5 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.
...
- Make sure that the 5.15.5 milestone is on the original PR and any others that have been pulled in

- If the PR(s) has any changes to the help/en/releasenotes directory, go through the steps to update the master if any section(s) were added, and to move notes and warnings to the 5.15.5 release note.  Merge these as needed to the release-5.15.5 and master branches
...
```
git checkout release-5.15.5
ant realclean compile
cd target
rm -f properties.5.15.5.zip

for x in `find classes -name \*.properties`; do
printf '%s\n' 0a '# from tag v5.15.5' . x | ex $x
done

find classes -name \*.properties | zip -@ properties.5.15.5.zip
cd ..
mkdir release
mv target/properties.5.15.5.zip release/
ls -lt release/
git checkout master
...
```
${EDITOR} ../website/releaselist ../website/releasenotes/jmri5.15.5.shtml scripts/HOWTO-distribution.md
```
...
- Disable the Jenkins [release-build project](https://builds.jmri.org/jenkins/job/testreleases/job/5.15.5/); this is so it doesn't fail after later steps
...
- Fill out form:
   ...
   - "tag version field" gets v5.15.5 (note leading lower-case "v"!)
   - @ branch: select the release-5.15.5 release branch
   - "Release title" field gets "Prod/Test Release 5.15.5"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```
[Release notes](https://jmri.org/releasenotes/jmri5.15.5.shtml)

Checksums:

File | SHA256 checksum
---|---
[JMRI.5.15.5+Rd7168b5b4e.dmg](https://github.com/JMRI/JMRI/releases/download/v5.15.5/JMRI.5.15.5+Rd7168b5b4e.dmg) | 3db9c22ae72fe890790fd34381594f0ae9f022e25badee196e81f27458545656
[JMRI.5.15.5+Rd7168b5b4e.exe](https://github.com/JMRI/JMRI/releases/download/v5.15.5/JMRI.5.15.5+Rd7168b5b4e.exe) | b51efbfad5ef8793018213f376d80a05f246726b8df3aa9f70ee2c40766c304f
[JMRI.5.15.5+Rd7168b5b4e.tgz](https://github.com/JMRI/JMRI/releases/download/v5.15.5/JMRI.5.15.5+Rd7168b5b4e.tgz) | 3f14443c422236d6eff65d15b3b7dfdd3795a3abff66ed9006ccdde0cc0184fb
...
```
git diff master...release-5.15.5
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues/new) to hold discussion with conventional title "Create Test Release 5.15.6". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 5.16 cycle. It's intended to be created from the `HEAD` of the `master` branch.
```
- Add the 5.15.6 milestone to the issue.

- Confirm that the tag for the current release (v5.15.5 for release 5.15.5) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-5.15.5) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
```
        ${EDITOR}  releasenotes/jmri5.15.5.shtml
```
...
Content:
    ...
    <https://www.jmri.org/releasenotes/jmri5.15.5.shtml#update>

    For more information on the issues, new features and bug fixes in 5.15.5 please see the release note:
    <https://www.jmri.org/releasenotes/jmri5.15.5.shtml>
...
- Close the [5.15.5 release GitHub Issue](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+%22Create+Test+Release+5.15.5%22) with a comment saying that
```
    JMRI 5.15.5 has been released. Files are available in the GitHub release section.
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.15.5 -n "JMRI.5.9.8+Rd144052.dmg" -f /var/lib/jenkins/jobs/testreleases/jobs/5.15.5/builds/4/archive/dist/release/JMRI.5.15.5+Rd144052.dmg
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.15.5 -n "JMRI.5.9.8+Rd144052.exe" -f /var/lib/jenkins/jobs/testreleases/jobs/5.15.5/builds/4/archive/dist/release/JMRI.5.15.5+Rd144052.exe
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.15.5 -n "JMRI.5.9.8+Rd144052.tgz" -f /var/lib/jenkins/jobs/testreleases/jobs/5.15.5/builds/4/archive/dist/release/JMRI.5.15.5+Rd144052.tgz
```
